...
local function EnsureOverlay()
    ...
    tex:SetVertexColor(0.18, 0.83, 0.75, 0)
    f._tex = tex
    _overlay = f
    return f
end
...
pulseFrame:SetScript("OnUpdate", function(self, dt)
    ...
    if t >= PULSE_DURATION or not _overlay then
        if _overlay then
            _overlay._tex:SetVertexColor(0.18, 0.83, 0.75, 0)
            _overlay:Hide()
    ...
    _overlay._tex:SetVertexColor(0.18, 0.83, 0.75, alpha)
end)
...
local function StartPulse(widgetFrame)
    ...
    hl._tex:SetVertexColor(0.18, 0.83, 0.75, 0)
    hl:Show()
    pulseFrame.elapsed = 0
    pulseFrame:Show()
end

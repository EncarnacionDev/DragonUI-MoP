...
Controls.Theme = {
    ...
    accentHex   = "2dd4bf",
    headerBg    = { 0.08, 0.09, 0.10, 0.9 },
    sectionBg   = { 0.11, 0.11, 0.13, 0.8 },
    sectionBorder = { 0.18, 0.18, 0.21, 1 },
    textNormal  = { 0.95, 0.95, 0.96, 1 },
    textDim     = { 0.61, 0.62, 0.65, 1 },
    textGold    = { 0.79, 0.80, 0.84, 1 },
    warning     = { 0.96, 0.65, 0.14, 1 },
    success     = { 0.20, 0.78, 0.35, 1 },
    danger      = { 1.0, 0.27, 0.23, 1 },
    widgetBg    = { 0.11, 0.11, 0.13, 1 },
    widgetBorder = { 0.18, 0.18, 0.21, 1 },
    buttonBg    = { 0.11, 0.11, 0.13, 1 },
    buttonHover = { 0.18, 0.83, 0.75, 0.3 },
...
local function SkinCheckBox(widget)
    ...
    if widget.check then
        widget.check:SetVertexColor(0.18, 0.83, 0.75, 1)
    end
    if widget.highlight then
        widget.highlight:SetTexture("Interface\\ChatFrame\\ChatFrameBackground")
        widget.highlight:SetVertexColor(0.18, 0.83, 0.75, 0.2)
    end
    -- Style text
    if widget.text then
...
local function SkinSlider(widget)
    ...
    if widget.slider and not widget.slider._dragonSkinned then
        ...
        if thumb then
            ...
            thumb:SetVertexColor(0.18, 0.83, 0.75, 1)
            thumb:SetWidth(12)
            thumb:SetHeight(12)
        end
...
local function SkinDropdown(widget)
    ...
    local function SyncDropdownButtonState()
        ...
        if widget.button and widget.button.GetHighlightTexture and widget.button:GetHighlightTexture() then
            if disabled then
                ...
                widget.button:GetHighlightTexture():SetVertexColor(0.18, 0.83, 0.75, 0.6)
            end
        end
    end
    ...
    if btn then
        ...
        if btn:GetHighlightTexture() then
            btn:GetHighlightTexture():SetVertexColor(0.18, 0.83, 0.75, 0.6)
        end
    end
    ...
    if widget.label then
        ...
        SafeSetFont(widget.label, 12, "")
        widget.label:SetTextColor(0.79, 0.80, 0.84)
    end
...
local function SkinButton(widget)
    ...
    if f then
        ...
        if not f._dragonHighlight then
            ...
            hl:SetVertexColor(0.18, 0.83, 0.75, 0.25)
            hl:SetAllPoints()
...
function Controls:AddSubTabs(parent, tabs, activeKey, onSelect, builders)
    ...
    for _, tab in ipairs(tabs) do
        ...
        if isActive then
            btn:SetText("|cff2dd4bf" .. tab.label .. "|r")
        else
...
local function SpellFilterPrint(msg)
    if addon and addon.Print then
        ...
        print("|cff2dd4bfDragonUI:|r " .. tostring(msg))
    end
end

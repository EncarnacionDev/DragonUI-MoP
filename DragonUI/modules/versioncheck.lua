...
local function SendVersion(channel)
    ...
    if not VALID_VERSION_CHANNELS[channel] then return end
...
local function BroadcastVersion()
    ...
    local numRaid = addon.GetNumRaidMembers()
    local numParty = addon.GetNumPartyMembers()
    ...
    -- BATTLEGROUND is not a valid SendAddonMessage channel in MoP 5.4.8.
    -- Battleground players are already covered by the RAID send above.
end

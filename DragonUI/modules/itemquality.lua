...
-- MoP inspect data tracking
local inspectDataReady = false
local currentInspectGUID = nil

local function UpdateCurrentInspectGUID()
    currentInspectGUID = InspectFrame and InspectFrame.unit and UnitGUID(InspectFrame.unit) or nil
end
...
local function InstallInspectHook()
    ...
    -- MoP 5.4.8 does not expose InspectPaperDollItemSlotButton_Update,
    -- InspectPaperDollFrame_OnShow, or InspectFrame_UnitChanged.
    -- Hook InspectFrame directly and refresh from INSPECT_READY.
    if InspectFrame then
        InspectFrame:HookScript("OnShow", function()
            if not IsModuleEnabled() then return end
            UpdateCurrentInspectGUID()
            inspectDataReady = false
            addon:After(0.1, UpdateAllInspectSlots)
            addon:After(0.6, UpdateAllInspectSlots)
            -- Fallback if INSPECT_READY never matches
            addon:After(2.0, function()
                if not inspectDataReady and InspectFrame:IsShown() and InspectFrame.unit then
...
eventFrame:SetScript("OnEvent", function(self, event, arg1)
    ...
    elseif event == "INSPECT_READY" then
        -- MoP uses INSPECT_READY with GUID payload (removed INSPECT_TALENT_READY in 4.1).
        if not IsModuleEnabled() then return end
        InstallInspectHook()
        currentInspectGUID = arg1
        local frameGUID = InspectFrame and InspectFrame.unit and UnitGUID(InspectFrame.unit)
        if frameGUID and arg1 == frameGUID then
            inspectDataReady = true
            addon:After(0.1, UpdateAllInspectSlots)
            addon:After(0.5, UpdateAllInspectSlots)
            addon:After(1.0, UpdateAllInspectSlots)
        end

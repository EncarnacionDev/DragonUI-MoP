...
local function ApplyNoopChangesImpl()
    ...
    pcall(function() MainMenuBar:EnableMouse(false) end)
    pcall(function() PetActionBarFrame:EnableMouse(false) end)
    if StanceBarFrame then
        pcall(function() StanceBarFrame:EnableMouse(false) end)
    end
    if BonusActionBarFrame then
        pcall(function() BonusActionBarFrame:EnableMouse(false) end)
        pcall(function() BonusActionBarFrame:SetScale(0.001) end)
    end
    ...
    if vehicleModuleEnabled and VehicleMenuBar then
        VehicleMenuBar:UnregisterAllEvents()
        VehicleMenuBar:Hide()
        VehicleMenuBar:SetAlpha(0)
        VehicleMenuBar:EnableMouse(false)
    else
        -- Vehicle module disabled or VehicleMenuBar does not exist (MoP).
    end
    ...
    if FCF_UpdateDockPosition then
        hooksecurefunc("FCF_UpdateDockPosition", function() end)
    end

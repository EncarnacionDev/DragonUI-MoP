...
local function GetAuraTiming(unitToken, index, isDebuff)
    ...
    if isDebuff then
        -- MoP 5.4.8: INCLUDE_NAME_PLATE_ONLY is not a valid UnitAura filter
        duration = select(6, UnitDebuff(unitToken, index))
        expirationTime = select(7, UnitDebuff(unitToken, index))
    else
        duration = select(6, UnitBuff(unitToken, index))
        expirationTime = select(7, UnitBuff(unitToken, index))
    end

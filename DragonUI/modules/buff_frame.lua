...
-- ConsolidatedBuffs was removed in MoP; guard for compatibility.
local original_CB_SetPoint, original_CB_ClearAllPoints
if ConsolidatedBuffs then
    original_CB_SetPoint = ConsolidatedBuffs.SetPoint
    original_CB_ClearAllPoints = ConsolidatedBuffs.ClearAllPoints
end
...
local function GetEnchantSlack()
    ...
    local hasMain, _, hasOff, _ = GetWeaponEnchantInfo()
    local enchants = (hasMain and 1 or 0) + (hasOff and 1 or 0)
    if enchants <= 0 then
        return 0
    end
    -- Prefer the live Blizzard count when available.
    if BuffFrame and BuffFrame.numEnchants and BuffFrame.numEnchants > 0 then
        return BuffFrame.numEnchants
    end
    return enchants
end
...
local function ReplaceBlizzardFrame(frame)
    ...
    -- ConsolidatedBuffs was removed in MoP; the root buff frame is anchored in Enable().
end
...
local function ApplyAuraScales()
    ...
    -- ConsolidatedBuffs was removed in MoP.
    if ConsolidatedBuffs then
...
local function RestoreWeaponEnchantsToChain()
    ...
    -- ConsolidatedBuffs was removed in MoP; anchor weapon enchants to BuffFrame.
    TemporaryEnchantFrame:ClearAllPoints()
...
function BuffFrameModule:SetupWeaponEnchantSeparation()
    ...
    if not dragonUIWeaponBuffFrame then
        ...
        addon:RegisterEditableFrame({
            ...
            blizzardFrame = TemporaryEnchantFrame or _G.TempEnchant1 or dragonUIWeaponBuffFrame,
...
function BuffFrameModule:Enable()
    ...
    -- ConsolidatedBuffs was removed in MoP; the root of the anchor chain is BuffFrame.
    ...
    -- PERMANENTLY OVERRIDE ConsolidatedBuffs positioning methods (WotLK only).
    ...
    local function GetBuffLayoutInfo()
        ...
        -- With no visible BuffButtons, anchor debuffs below weapon enchants
        -- instead of the empty mover frame. ConsolidatedBuffs was removed in MoP.
    ...
    -- ========================================================================
    -- HELPER: Re-anchor the buff root frame and weapon enchant chain.
    -- Blizzard code (UIParent_ManageFramePositions, etc.) may reposition
    -- BuffFrame; this restores our custom placement.
    -- ========================================================================
    local function RestoreBuffChain()
        -- ConsolidatedBuffs was removed in MoP; re-anchor the root buff frame.
        if dragonUIBuffFrame then
            original_BuffFrame_ClearAllPoints(BuffFrame)
            original_BuffFrame_SetPoint(BuffFrame, "TOPRIGHT", dragonUIBuffFrame, "TOPRIGHT", 0, 0)
        end
        if ConsolidatedBuffs and dragonUIBuffFrame then
            original_CB_ClearAllPoints(ConsolidatedBuffs)
            original_CB_SetPoint(ConsolidatedBuffs, "TOPRIGHT", dragonUIBuffFrame, "TOPRIGHT", 0, 0)
        end
        -- When weapon enchants are separated, TemporaryEnchantFrame is managed
        -- by the weapon enchant system — do NOT re-anchor it here.
        if weaponEnchantsAreSeparated then return end
        -- In MoP, weapon enchants anchor to BuffFrame.
    ...
    local function AnchorFirstBuff(button, slack)
        ...
        -- ConsolidatedBuffs was removed in MoP; anchor to the mover frame.
        button:ClearAllPoints()
        button:SetPoint("TOPRIGHT", dragonUIBuffFrame, "TOPRIGHT", 0, 0)
    end
    ...
    if not BuffFrameModule._hookedBuffAnchors then
        ...
        hooksecurefunc("BuffFrame_UpdateAllBuffAnchors", function()
            ...
            --    sharpening stones, etc.) to follow BuffFrame.
            --    In MoP ConsolidatedBuffs was removed, so the root is BuffFrame.
            --    SKIP this when weapon enchants are separated — they have
            --    their own independent anchor managed by the weapon frame.
            if not weaponEnchantsAreSeparated and TemporaryEnchantFrame then
                TemporaryEnchantFrame:ClearAllPoints()
                TemporaryEnchantFrame:SetPoint("TOPRIGHT", BuffFrame, "TOPRIGHT", 0, 0)
            end

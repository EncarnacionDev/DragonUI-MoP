...
local function InstallInspectHooks()
    ...
    if InspectPaperDollFrame_OnShow then
        hooksecurefunc("InspectPaperDollFrame_OnShow", function()
            ...
            HideInspectTexts()
            -- Safety net: draw anyway if INSPECT_READY never arrives
            Debounce("inspectfallback", 1.5, function()
                if not inspectDataReady and InspectFrame and InspectFrame:IsShown() then
    ...
    -- Retargeting reuses the open frame; reset state when the inspected unit changes.

...
local function RemoveBlizzardFrames()
    ...
    local blizzFrames = {MainMenuBarPerformanceBar, MainMenuBarTexture0, MainMenuBarTexture1, MainMenuBarTexture2,
                         ...
                         StanceBarLeft, StanceBarMiddle, StanceBarRight, PossessBackground1,
                         PossessBackground2}
...
-- Blizzard re-anchors the native action bars at many points (spirit release,
-- entering/leaving a battleground, combat transitions, targeting, etc.), so
-- chasing individual events isn't reliable. Re-anchor the bars once a second
-- whenever the player is out of combat; this covers every case without touching
-- the editor frames. Skipped in combat (taint) and in editor mode (dragging).
...
local function ApplyMainbarsSystem()
    ...
    -- Stop Blizzard's UIParent_ManageFramePositions from re-anchoring the
    -- secondary action bars (same approach as Bartender4). Without this,
    -- entering a battleground, combat transitions, etc. move MultiBarBottomLeft
    -- out of our layout.
...
local function InitializeMainbars()
   ...
   -- Specific function to disable MainMenuBarMaxLevelBar
    ...
    mainbarsEventFrame:SetScript("OnEvent", function(self, event, unitOrAddon)
        ...
        elseif event == "PLAYER_REGEN_ENABLED" then
            -- Reposition when combat ends. Defer a tick so InCombatLockdown()
            -- has fully cleared before we re-anchor the bars.
            if IsModuleEnabled() and addon.core and addon.core.ScheduleTimer then
...
local MIGRATED_VISIBILITY_BARS = {
    ...
    -- Hover over the secure replacement buttons, not the hidden native ones, to avoid taint.
    {
        key = "micro",
        frame = function() return _G.pUiMicroMenu end,
        hoverFrameResolver = function()
            local frames = {}
            for _, name in ipairs(addon.MicroMenuButtonNames or {}) do
                local native = _G[name]
                local rep = native and addon.GetMicroMenuReplacementButton(native)
                if rep then table.insert(frames, rep) end
            end
            return frames
        end,
        mouseSafeInCombat = true
...
local function SyncMigratedBarVisibility(bar)
    ...
    if bar.hoverFrameResolver then
        for _, btn in ipairs(bar.hoverFrameResolver()) do
            if btn then table.insert(hoverFrames, btn) end
        end
    elseif bar.buttonPrefix then
        for i = 1, 12 do

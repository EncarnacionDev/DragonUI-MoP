...
behaviors._ExemptLFGFromSexyMapFade = function()
    ...
    -- Already applied?
    if lfgFrame._DragonUI_SexyMapFadeExempt then return end
    lfgFrame._DragonUI_SexyMapFadeExempt = true

    local sexyMapObj = _G["SexyMap"]
    -- Unregister from SexyMap's hover system
    if sexyMapObj and sexyMapObj.UnregisterHoverButton then
        sexyMapObj:UnregisterHoverButton(lfgFrame)
    end
    -- Force full alpha
    lfgFrame:SetAlpha(1)

    -- Safety net: hook SetAlpha to prevent SexyMap from fading it
    local origLFGSetAlpha = lfgFrame.SetAlpha
    lfgFrame.SetAlpha = function(self, alpha)

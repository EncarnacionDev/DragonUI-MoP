...
CreateDragonUIButton = function()
    ...
    if useCustom then
        ...
        bgTex:SetVertexColor(0.70, 0.10, 0.10)
    else
        local WHITE = "Interface\\Buttons\\WHITE8X8"
        bgTex:SetTexture(WHITE)
        bgTex:SetBlendMode("ADD")
        bgTex:SetVertexColor(0.60, 0.10, 0.10, 1.0)
    end
    ...
    -- RGB tuples for interpolation (red, matching other Game Menu buttons)
    local NRM   = {0.70, 0.10, 0.10}  -- bgTex base color (custom texture)
    local HOV   = {0.90, 0.20, 0.20}  -- bgTex hover color (custom texture)
    local OVR   = {0.60, 0.10, 0.10}  -- bgTex base color (solid fallback)
    local OVR_H = {0.80, 0.15, 0.15}  -- bgTex hover color (solid fallback)
